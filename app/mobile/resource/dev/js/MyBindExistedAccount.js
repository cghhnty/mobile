function MyBindExistedAccount(opts) {
    ...
    me.find(".my-bind-existed-account-send-authcode").click(function() {
        if (!/\d{11}/.test(me.phone)) return modal("请输入正确的手机号码");
        me.countdownStart();
        api.post("member/sendAuthcode", {
            phone: me.phone
        }, function(err, result) {
            if (err) {
                me.countdownStop();
    ...
    me.find("form").submit(function(e) {
        ...
        if (!me.authcode) return modal("请输入验证码");
        me.submiting = true;
        api.post("member/verifyAuthcode", {
...
//# sourceMappingURL=MyBindExistedAccount.js.map