...
Url.formatSearch = function(query) {
    ...
    for (var p in query) {
        [].concat(query[p]).forEach(function(val) {
            ...
            if (val !== "") search += "=" + encodeURIComponent(val);
        });
    }
...
Url.prototype = {
    format: function() {
        ...
        if (this.query) url += Url.formatSearch(this.query);
    ...
    removeQuery: function() {
        for (var i = arguments.length - 1; i >= 0; i--) delete this.query[arguments[i]];
        return this;
    },
...
function extendClass(SubClass, SuperClass) {
    ...
    for (var prop in proto) SubClass.prototype[prop] = proto[prop];
    return SubClass;
}
...
function handleError(err, handlers) {
    if (!err) return;
    var h = {};
    for (var k in handleError.handlers) h[k] = handleError.handlers[k];
    for (k in handlers) h[k] = handlers[k];
    handlers = h;
    handleError.bindElements(err);
    if (handlers[err.code]) handlers[err.code](err); else if (handlers._default) handlers._default(err);
    if (handlers._finally) handlers._finally(err);
}
...
handleError.bindElements = function(err) {
    ...
    for (var i = 0; i < elems.length; i++) {
        ...
        if (fn && window[fn] && window[fn].constructor == Function) window[fn].call(elem, err);
    }
};
...
inc.get = function(url) {
    ...
    if (obj && obj.type == "text") return obj.content; else return null;
};

inc._resolveUrl = function(url) {
    if (!/^(https?:|\/\/)/.test(url)) {
        if (inc.base) {
            if (url.charAt(0) != "/") url = inc.base + "/" + url;
...
function Renderer(elem, data) {
    if (elem.constructor == String) elem = document.querySelector(elem);
    if (elem.renderInst) {
...
Renderer.forEach = function(list, cb) {
    ...
    if (list.constructor == Array || "length" in list) {
        for (var i = 0; i < list.length; i++) cb(list[i], i, list);
...
Renderer._eval = function(exp, baseObj) {
    ...
    if (!depth) {
        ...
        var tmp = new Function("baseObj", "			try {				with(baseObj) {					var val = " + exp + ";					var obj = " + depth + ";				}				return {obj: obj, val: val};			} catch(e) {return null}")(baseObj);
        if (tmp) {
            return {
                obj: tmp.obj,
                prop: prop,
...
Renderer.val = function(val, type) {
    if (type == "number") val = val == null ? null : Number(val); else if (type == "boolean") val = Boolean(val);
    return val;
};
...
Renderer.prototype = {
    compile: function(elem, setting) {
        ...
        for (var j = 0; j < elem.attributes.length; j++) {
            ...
            if (attrName == "rn-eval") {
                ...
                if (attrVal.indexOf("{{") != -1 && attrVal.indexOf("}}") != -1) {
                    attrVal = attrVal.split("{{").join("\x00").split("}}").join("").replace(/(^|\x01)[^\x00]*/g, function(s) {
                        return s.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
                    }).replace(/\x00([^\x01]*)\x01/g, '" + __val($1) + "');
                    attrVal = '"' + attrVal + '"';
        ...
        if (fn || hasBinds) {
            ...
            if (hasBinds) nodeSetting.binds = binds;
                ...
                if (node.nodeValue.indexOf("{{") != -1 && node.nodeValue.indexOf("}}") != -1) {
                    node.nodeValue.split(/\{\{|\}\}/).forEach(function(val, i) {
                        if (i % 2 == 0) {
                            ...
                            var id = setting.nodes.push({
                                renderer: new Function("_data", "_scope", "_value", "_key", "_list", "_parent", fn)
                            }) - 1;
    ...
    init: function(elem, scopeSetting, scopeInst) {
        ...
        Array.prototype.slice.call(elem.childNodes).forEach(function(node) {
            if (node.nodeType == 1 && !node.hasAttribute("rn-scope")) {
    ...
    resolve: function(elem, scopeInst, data, scopeData, value, key, list, parent) {
        ...
        scopeInst.nodes.forEach(function(n) {
            ...
            if (n.setting.renderer) n.setting.renderer.call(n.node, data, scopeData, value, key, list, parent);
        });
        ...
        scopeInst.foreachScopes.forEach(function(scope) {
            ...
            Renderer.forEach(list, function(value, key) {
                ...
                if (elem != nextSibling) scope.placeholder.parentNode.insertBefore(elem, nextSibling);
        ...
        function createForeachElement(value, scope) {
            ...
            if (value instanceof Object) {
                ...
                if (!value._foreachElements[scope.id]) value._foreachElements[scope.id] = [];
                value._foreachElements[scope.id].push(elem);
            }
            return elem;
        }
    ...
    bindData: function(elem, elemProp, data, dataProp) {
        ...
        for (var i = 0; i < items.length; i++) {
            var item = items[i];
            if (item.obj == e.obj && item.prop == e.prop) return;
        }
        ...
        if (binds.initVal != null) setVal(binds.initVal);
        function getVal() {
            if (elem != e.obj || e.prop != "value") return e.obj[e.prop];
            var valType = elem.getAttribute("rn-value-type") || elem.type;
            if (valType == "checkbox" && elem.getAttribute("value") == null) valType = "boolean";
            if (elem.type == "radio") {
                for (var i = 0; i < items.length; i++) {
                    var el = items[i].elem;
                    if (el.checked) return Renderer.val(el.value, valType);
                }
                return null;
                ...
                if (items.length == 1) {
                    ...
                    for (var i = 0; i < items.length; i++) {
                        var el = items[i].elem;
                        if (el.checked) values.push(Renderer.val(el.value, valType));
                    }
                    return values;
                }
...
function render(elem, data, renderChildScopes) {
    ...
    if (renderChildScopes) r.renderChildScopes();
}
...
function res(path) {
    ...
    if (res.data.hash[path]) return base + path + "?" + res.data.hash[path];
...
function date(timestamp) {
    if (!timestamp) timestamp = Date.now(); else if (timestamp < 1e10) timestamp *= 1e3;
...
function wxConfig(jsApiList) {
    if (wxConfig.promise) return wxConfig.promise;
    var def = $.Deferred();
    ...
    inc("http://res.wx.qq.com/open/js/jweixin-1.0.0.js", function() {
        var url = new Url();
        url.hash = "";
        url = url.format();
        api.get("weixin/jsApiSign", {
            url: url
        }).fail(function(err) {
            def.reject(err);
...
function wxOnMenuShare(data) {
    ...
    if (!data.link) data.link = location.href;
    if (!data.imgUrl) data.imgUrl = $("img").prop("src") || res("img/common/logo@200x200.png");
    function cb() {
        $(".common-wx-share-overlay").hide();
    }
    if (!data.success) data.success = cb;
    if (!data.cancel) data.cancel = cb;
    var url = new Url(data.link);
    url.removeQuery("loginWeixin", "loginMember", "loginToken", "loginExpire");
    if (member) url.query.referrer = member.id;
...
(function() {
    ...
    query = new Url().query;
    if (!standalone) {
        inc("lib/bootstrap/css/bootstrap.css", "lib/bootstrap/js/bootstrap.js");
    }
    inc("lib/font-awesome/css/font-awesome.css", "css/common.css", "html/common.html", function() {
    ...
    if (screenWidth) $("html").width(screenWidth);
    if (masterpage) {
