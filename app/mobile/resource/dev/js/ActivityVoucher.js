function ActivityVoucher(opts) {
    ...
    me.find(".activity-voucher-stores-toggle").click(function() {
        if (me.$stores.hasClass("activity-voucher-collapse")) {
            me.$stores.removeClass("activity-voucher-collapse").addClass("activity-voucher-expand").animate({
                height: me.storesHeightExpand
            });
            me.$storesToggleIcon.removeClass("fa-angle-down").addClass("fa-angle-up");
        } else {
            me.$stores.removeClass("activity-voucher-expand").addClass("activity-voucher-collapse").animate({
    ...
    me.find(".activity-voucher-take").click(function() {
        if (me.voucher) return;
        api.post("activity/createVoucher", {
            id: me.activityId
        }).done(function(err, voucher) {
            me.voucher = voucher;
            me.render();
        });
    });
    api.get("activity/getVoucherActivity", {
        id: query.id
...
//# sourceMappingURL=ActivityVoucher.js.map