function ActivityHistory(opts) {
    var me = this;
    me.init(opts);
    if (!me.type && query.type) me.type = query.type;
    document.title = "中奖记录";
    me.$root.on("click", ".activity-history-item", function() {
        $(this).find(".activity-history-details").animate({
            height: "toggle"
        });
...
ActivityHistory.prototype = {
    ...
    load: function() {
        var me = this;
        return api.get("activity/getHistories", {
            type: me.type
        }).done(function(err, histories) {
            ...
            for (var i = 0; i < me.histories.length; i++) {
                ...
                if (history.card.status == "ACTIVE" && (!history.card.expirationTime || now < history.card.expirationTime)) me.activeTickets.push(history); else me.unavailableTickets.push(history);
