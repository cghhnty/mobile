function MyCenter(opts) {
    ...
    $.when(api.get("report/getViewConsumeNum").done(function(err, result) {
        console.log(result);
        ...
        memberEx.cards.forEach(function(card) {
            if (card.expirationTime && card.expirationTime < now || card.status != "ACTIVE") {
                return;
            } else if (card.type in {
                BALANCE_ACCOUNT: 1
            }) {
                if (!balanceLastModified || card.lastModified > balanceLastModified) balanceLastModified = card.lastModified;
            }
            if (card.type == "VOUCHER_CARD") {
                me.voucherCardsCount++;
                if (!voucherLastModified || card.lastModified > voucherLastModified) voucherLastModified = card.lastModified;
            }
            if (card.type in {
                STAMP_CARD: 1,
                PERIOD_CARD: 1,
                ...
                me.cardsCount++;
                if (!cardLastModified || card.lastModified > cardLastModified) cardLastModified = card.lastModified;
            }
        });
        ...
        result.forEach(function(promotion) {
            ...
            list.push({
                content: promotionDom.get(0).outerHTML
            });
        });
        if (list.length) {
...
//# sourceMappingURL=MyCenter.js.map