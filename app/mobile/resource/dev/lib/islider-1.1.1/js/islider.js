(function() {
    ...
    iSlider = function() {
        ...
        iSlider.prototype._setting = function() {
            ...
            this.duration = opts.duration || 2e3;
    ...
    plugins_islider_animate = function(iSlider) {
        var extendAnimation = {
            rotate: function(dom, axis, scale, i, offset) {
                ...
                dom.style.webkitTransform = "rotate" + rotateDirect + "(" + 90 * (offset / scale + i - 1) + "deg) translateZ(" + .888 * scale / 2 + "px) scale(0.888)";
            },
            ...
            depth: function(dom, axis, scale, i, offset) {
                var zoomScale = (4 - Math.abs(i - 1)) * .18;
                this.wrap.style.webkitPerspective = scale * 4;
            ...
            card: function(dom, axis, scale, i, offset) {
                ...
                if (i === 1) {
                    ...
                    dom.style.zIndex = offset > 0 ? (1 - i) * absoluteOffset * 1e3 : (i - 1) * absoluteOffset * 1e3;
                }
                ...
                var zoomScale = dom.cur ? 1 - .2 * Math.abs(i - 1) - Math.abs(.2 * offset / scale).toFixed(6) : 1;
                dom.style.webkitTransform = "scale(" + zoomScale + ", " + zoomScale + ") translateZ(0) translate" + axis + "(" + ((1 + Math.abs(i - 1) * .2) * offset + scale * (i - 1)) + "px)";
            }
        };
