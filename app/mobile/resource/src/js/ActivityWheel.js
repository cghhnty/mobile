function ActivityWheel(opts) {
	...
	me.find('.activity-wheel-stores-toggle').click(function() {
		if (me.$stores.hasClass('activity-wheel-collapse')) {
			me.$stores.removeClass('activity-wheel-collapse').addClass('activity-wheel-expand').animate({height: me.storesHeightExpand});
			me.$storesToggleIcon.removeClass('fa-angle-down').addClass('fa-angle-up');
		} else {
			me.$stores.removeClass('activity-wheel-expand').addClass('activity-wheel-collapse').animate({height: me.storesHeightCollapse});
...
ActivityWheel.prototype = {
	...
	draw: function() {
		...
		if (me.tried)
			return modal('亲~你已经抽过奖了!\n关注微信号"' + store.weixinConfig.name + '"获得更多抽奖机会');

		me.running = true;

		api.post('activity/draw', {id: query.id}).fail(function() {
			stop = false;
		}).done(function(err, ticket) {
			me.ticket = ticket;

			api.post('activity/open', {id: me.ticket.id}).fail(function() {
				stop = false;
			}).done(function() {
			...
			setTimeout(function() {
				...
				if (stop === false) {
					...
					if (j != a) {
						...
						} else if (min && (last <= me.spinMinLast || stop == undefined)) { // 保持高速旋转
							last += min;
						} else { // 减速
							if (!scale) {
