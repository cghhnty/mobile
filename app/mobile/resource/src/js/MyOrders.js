...
MyOrders.prototype = {
	...
	load: function() {
		...
		return api.get('report/getTransactionFlowReport4Member', {cardId: query.cardId}).done(function(err, orders) {
			me.orders = orders;
	...
	checkOnlyCards : function(value){//办卡单
		if(!!value){
			value.forEach(function(val,index){
				var isOnlycard = true;
				if(!!val){
					val.itemList.forEach(function(val,index){
						if(!(val.type in {STAMP_CARD:1,PERIOD_CARD:1,DISCOUNT_CARD:1,BALANCE_CARD:1})){
		           			isOnlycard = false;
						}
					});
					if (isOnlycard){
