function ShouqianbaQueryOrderDetail(opts) {
  ...
  if (orderSn) {
    api.get('shouqianba/getPosOrder', {TxSn: orderSn}).done(function (err, order) {
      if (err || !order) {
        console.log(err);
        $('#fail-container').show();
        $('#detail-container').hide();
        ...
        if (isNaN(receiptAmount) || receiptAmount == 0.0) {
          me.trade_amount = order.total_fee;
        } else {
          me.trade_amount = (receiptAmount / 100.0).toFixed(2) + "";
        }
